...
bool RinhaController::checkIfClientExists(const std::string& clientIdAsStr) {
    int clientIdAsInt;
    try {
        clientIdAsInt = std::stoi(clientIdAsStr);
    ...
    return !(clients_.empty() || clients_.find(clientIdAsInt) == clients_.end());
}
